...
class UserRepository: UserInterface {
    ...
    override fun getUserByID(id: Int): User? {
        val selectAllSql = "SELECT * FROM user WHERE id = ?"
        return jdbcTemplate.queryForObject(selectAllSql, UserRowMapper(), id)
    }

    override fun addNewUser(userModel: User) {
        val addQuery = "INSERT INTO user (name, email, contact_number) values (?,?,?)"
    ...
    override fun updateUser(userModel: User, id: Int) {
        val updateQuery = "UPDATE user SET name=?,email=?, contact_number=? WHERE id=?"
